    ...
    <div class="space-y-6">
        ...
        <div class="fi-section rounded-xl bg-white shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
            <div class="fi-section-content p-6">
                ...
                <div
                    class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                    <div>
                        <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Dibuat Oleh</p>
                        <p class="text-gray-900 dark:text-white">{{ $record->createdBy->name ?? '-' }}</p>
                        <p class="text-xs text-gray-500">{{ $record->created_at->format('d/m/Y H:i') }}</p>
                    </div>
                    @if ($record->submittedBy)
                        <div>
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Disubmit Oleh</p>
                            <p class="text-gray-900 dark:text-white">{{ $record->submittedBy->name }}</p>
                            <p class="text-xs text-gray-500">{{ $record->submitted_at?->format('d/m/Y H:i') }}</p>
                        </div>
                    @endif
                    @if ($record->approvedBy)
                        <div>
                            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Diapprove Oleh</p>
                            <p class="text-gray-900 dark:text-white">{{ $record->approvedBy->name }}</p>
                            <p class="text-xs text-gray-500">{{ $record->approved_at?->format('d/m/Y H:i') }}</p>
                        </div>
                    @endif
                </div>
        ...
        {{-- Summary Stats - 3 columns horizontal like stok opname items --}}
