    ...
    protected function getHeaderActions(): array
    {
        return [
            Actions\Action::make('lihat_riwayat')
                ...
                ->color('info')
                ->visible(fn() => Auth::user()->can('lihat_seluruhnya_riwayat_produksi') || Auth::user()->can('lihat_sebagian_riwayat_produksi'))
                ->modalHeading('Riwayat Produksi')
                ...
                ->modalContent(function () {
                    ...
                    if (!Auth::user()->can('lihat_seluruhnya_riwayat_produksi') && Auth::user()->can('lihat_sebagian_riwayat_produksi')) {
                    ...
                    if ($records->isEmpty()) {
                        return new HtmlString('<div class="text-center py-8 text-gray-500">Tidak ada riwayat produksi</div>');
                    }
                    ...
                    $html .= '<th class="px-4 py-2 text-left">Mesin</th>';
                    ...
                    foreach ($records as $record) {
                        ...
                        $html .= '<td class="px-4 py-2">' . e($record->produkProses?->mesin?->nama ?? '-') . '</td>';
