    ...
    protected function getHeaderActions(): array
    {
        return [
            Actions\Action::make('lihat_riwayat')
                ->label('Lihat Riwayat Finishing')
                ->icon('heroicon-o-clock')
                ->color('info')
                ->visible(fn() => Auth::user()->can('lihat_seluruhnya_riwayat_finishing') || Auth::user()->can('lihat_sebagian_riwayat_finishing'))
                ->modalHeading('Riwayat Finishing')
                ...
                ->modalContent(function () {
                    $query = TransaksiProses::query()
                        ...
                        ->whereHas('produkProses', function($q) {
                            $q->where('produk_proses_kategori_id', ProdukProsesKategori::finishingId()); // Finishing/Addon
                        })
                        ->with([
                    ...
                    if (!Auth::user()->can('lihat_seluruhnya_riwayat_finishing') && Auth::user()->can('lihat_sebagian_riwayat_finishing')) {
                    ...
                    if ($records->isEmpty()) {
                        return new HtmlString('<div class="text-center py-8 text-gray-500">Tidak ada riwayat finishing</div>');
                    }
                    ...
                    $html .= '<th class="px-4 py-2 text-left">Proses Finishing</th>';
